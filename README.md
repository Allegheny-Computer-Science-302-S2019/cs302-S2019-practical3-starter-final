...
# cs302-S2019-practical3-starter

Designed for use with [GitHub Classroom](https://classroom.github.com/), this
repository contains the starter for Practical 3 in Computer Science 302. Since
...
practical assignments on the course web site.
...
- src/www/travels.html:
  ...
  - Contains exactly eight uses of an `h3` tag with various labels as content
  - Contains exactly four references to the `about.html` file
  - Contains exactly four references to the `software.html` file
  - Contains exactly four uses of a `_blank` annotation for link opening
  - Contains exactly two uses of a `ul` tag for creating an unordered list
  - Contains exactly two uses of a `ol` tag for creating an ordered list
  - Contains exactly two uses of a `em em-` for displaying emojis in the HTML
  - Contains exactly one reference to the `github.css` file for formatting content
  - Contains exactly one use of the `html` and `body` and `footer` tags
...
```
git remote add download git@github.com:Allegheny-Computer-Science-302-S2019/cs302-S2019-practical3-starter.git
```
